...
pub(crate) fn is_near_bindgen_wrapped_or_marshall(item: &ItemFn) -> bool {
    // Since rustc 1.87, `#[cfg]` attributes are evaluated and stripped before attribute
    // proc macros run, so the cfg-based heuristic below no longer sees near_bindgen's
    // `#[cfg(target_arch = "wasm32")]` marker on generated wrapper functions. The generated
    // wrapper/marshall functions never take `self`, while plugin macros are only meaningful
    // on methods, so treat any function without a `self` receiver as generated.
